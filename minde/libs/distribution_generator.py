...
class Agent:
    ...
    def __init__(self, dim_x: int, dim_y: int, params: Optional[np.array] = None) -> None:
        ...
        dim_x: dimension of first marginal
        dim_y: dimension of second marginal
        """
        if params is None:
...
class EvolutionTask:
    ...
    def __init__(self, target_mutinfo: float, dim_x: int, dim_y: int, scale: float = 1.0, loc: float = 0.0, strategy='comma', mu=25, population_size=50) -> None:
        self.scale = scale
        self.loc = loc
        self.mu = mu
